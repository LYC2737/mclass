pipline {
    ...
    tools {
        maven 'maven 3.9.11' // Jenkins에 등록된 Maven 3.9.11 버전을 사용
    }

    environment{
        // 배포에 필요한 변수 설정
        DOCKER_IMAGE = "demo-app" //도커이미지 이름
        ...
        SSH_CREDENTIALS_ID = "cbc64655-fbde-4b1d-9532-2864efe73825" //Jenkins SSH 자격 증명 ID
    }
    ...
    stages {
        ...
        stage('Prepare Jar'){
            steps{
                ...
                sh 'cp target/demo-0.0.1-SNAPSHOT.jar ${JAE_FILE_NAME}'
            }
        }

        stage('Copy to Remote Server'){
            steps {
                //Jenkins가 원격 서버에 SSH 접속할 수 있도록 sshagent               
